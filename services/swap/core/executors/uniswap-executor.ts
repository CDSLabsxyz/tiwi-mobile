 ...
 * 
 * Executes swaps using Uniswap V2 / SushiSwap on Ethereum and other supported chains.
 */
...
// Uniswap V2 Router ABI (includes getAmountsOut + fee-on-transfer support)
...
/**
 * Uniswap executor implementation
 */
export class UniswapExecutor extends EVMDEXExecutor implements SwapRouterExecutor {
  /**
   * Check if this executor can handle the given route
   */
  canHandle(route: RouterRoute): boolean {
    return route.router === 'uniswap';
  }
   ...
   * ✅ EXACTLY matches tiwi-test's getUniswapV2SwapData
   */
  protected buildSwapData(
    ...
    // This matches Uniswap/SushiSwap UI behavior - always use supporting functions unless explicitly disabled
